...
name: ecc
description: >
  Layer ECC (github.com/affaan-m/ECC v2.2.0) instalado en Matcher: reglas de estilo, testing,
...
1. Correr el gate real del repo y **pegar la salida cruda**, no un resumen:
   ```bash
   ruff check . && python3 -m pytest -q tests/   # job `motor` de CI
cd webapp/frontend && npm ci && npm run build   # job `frontend`
   ```
   CI tiene tres jobs: `motor` (ruff + pytest), `frontend` (build de Vite) y `demo` (humo: el servidor levanta y responde). Los tres tienen que estar verdes — que pytest pase no prueba que el servidor arranque.
